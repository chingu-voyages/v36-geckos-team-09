...
module.exports = class FlashcardsController {
    static async apiGetFlashcards(req, res, next) {
        ...
        const { flashcardsList, totalNumFlashcards } =
            await FlashcardsDAO.getFlashcards({
                filters,
                page,
                flashcardsPerPage,
            });
    ...
    static async apiGetFlashcardByCollection(req, res, next) {
        try {
            ...
            let flashcardsCollection =
                await FlashcardsDAO.getFlashcardByCollection(collection_name);
            if (!flashcardsCollection) {
                res.status(404).json({
                    error: 'Flashcards Collection Not found',
                });
                return;
            }
            let response = {
    ...
    static async apiDeleteCollection(req, res, next) {
        try {
            ...
            const flashcardResponse = await FlashcardsDAO.deleteCollection(
                collection_name,
            );
    ...
    static async apiPostCollection(req, res, next) {
        try {
            ...
            const FlashcardResponse = await FlashcardsDAO.createCollection(
                collection_name,
            );
            res.json({ status: 'success' });
        } catch (e) {
