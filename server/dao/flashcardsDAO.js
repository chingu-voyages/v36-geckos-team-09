...
export default class FlashcardsDAO {
    static async injectDB(conn) {
        ...
        try {
            flashcards = await conn
                .db(process.env.FLASHCARDS_NS)
    ...
    static async getFlashcards({
        ...
        const displayCursor = cursor
            .limit(flashcardsPerPage)
            .skip(flashcardsPerPage * page);
    ...
    static async getFlashcardByCollection(collection_name) {
        try {
            ...
            console.error(
                `Something went wrong in getFlashcardsByCollection: ${e}`,
            );
            throw e;
        }
    }
